...
const CLAIM_LIMIT = 1_250;
const SENDS_PER_SECOND = 25; // Safely below Telegram's ~30/s free broadcast ceiling.

const wait = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));
const textOnlyBroadcasts = new Set<number>();

async function deliver(broadcast: any, recipient: any) {
  const photo = textOnlyBroadcasts.has(Number(broadcast.id)) ? undefined : broadcast.image_path || undefined;
  let result = await sendTelegramMessage(recipient.telegram_id, broadcast.message_html, {
  ...
  if (!result?.ok && photo && /failed to get http url content/i.test(String(result?.description || ""))) {
    const firstImageFailure = !textOnlyBroadcasts.has(Number(broadcast.id));
    textOnlyBroadcasts.add(Number(broadcast.id));
    if (firstImageFailure) console.warn("Platform broadcast image unavailable; retrying text-only", { broadcast_id: broadcast.id, code: "BROADCAST_IMAGE_FETCH_FAILED" });
    result = await sendTelegramMessage(recipient.telegram_id, broadcast.message_html, {
      parse_mode: "HTML", timeoutMs: 10_000,
      reply_markup: broadcast.button_text && broadcast.button_url ? { inline_keyboard: [[{ text: broadcast.button_text, url: broadcast.button_url }]] } : undefined,
    });
  }
  if (result?.ok) {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const broadcast = broadcasts[0]; if (!broadcast) return NextResponse.json({ processed: 0 });
