...
export async function GET(req: NextRequest) {
  ...
  const lock = await acquireCronLock("settle-views", 1800);
  if (!lock) {
    return NextResponse.json({ success: false, message: "View settlement cron is already running" }, { status: 409 });
  }
  ...
  try {
    ...
    const [throttleResult]: any = await pool.query(
      `UPDATE settings
       SET value = ?
       WHERE \`key\` = 'last_settlement_views_run'
    ...
    try {
      for (let post of postsToSettle) {
        ...
        try {
          ...
          post = lockedPost;
          const lockedNewViews = Number(post.current_total_views || 0) - Number(post.settled_views || 0);
          if (lockedNewViews <= 0) {
            await conn.rollback();
            continue;
          }
          const lockedAdvertiserPaid = (lockedNewViews * Number(post.cpm || 0)) / 1000;
          ...
          const safetyCheck = await recordPayoutSafetyCheck({
            ...
            metadata: { post_id: post.post_id, views: lockedNewViews },
          });
          ...
          await conn.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'locked')
          `, [post.post_id, post.campaign_id, post.advertiser_id, post.channel_id, post.publisher_id, lockedNewViews, lockedAdvertiserPaid, lockedPublisherReward]);
          ...
          await conn.commit();
          results.push({ post_id: post.post_id, settled_views: lockedNewViews, paid: lockedAdvertiserPaid, reward: lockedPublisherReward });
        } catch (err) {
          await conn.rollback();
