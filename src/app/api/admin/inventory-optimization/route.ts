...
async function leaderboard(entityType: "miniapp" | "channel" | "bot") {
  ...
  const [rows]: any = await pool.query(`
    SELECT
      ...
      COALESCE((SELECT COUNT(*) FROM broadcast_deliveries bd WHERE bd.bot_id = b.id AND bd.status = 'sent' AND bd.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)), 0) as impressions_7d,
