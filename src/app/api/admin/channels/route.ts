...
export async function GET(request: Request) {
  ...
  try {
    let query = `
      SELECT c.*, u.first_name, u.last_name, u.username AS owner_username, u.telegram_id
      FROM channels c
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (action === "reject") {
      await pool.query("UPDATE channels SET status = 'rejected' WHERE id = ?", [id]);
      return NextResponse.json({ success: true });
    }

    if (action === "approve") {
      await pool.query("UPDATE channels SET status = 'active' WHERE id = ?", [id]);
      return NextResponse.json({ success: true });
    }
