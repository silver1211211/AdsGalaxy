...
export default function AudienceAnalyticsPage() {
  ...
  const visibleAudiences = audiences.filter((audience) => audience.key !== "global" && audience.key !== "unclassified");
  ...
  return <AdminLayout>
    <div className="space-y-5 pb-8">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-black uppercase tracking-widest text-[#0c9de8]">Channel inventory</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Audience Analytics</h1><p className="mt-2 text-sm text-slate-500">{loading && totalChannels === 0 ? "Loading active channel capacity." : `${number(totalChannels)} active channels across six regional audiences.`}</p></div>
      ...
      {scope && <section className="grid gap-3 sm:grid-cols-3">
      ...
      <div className="grid min-w-0 gap-3 xl:grid-cols-2">
        {visibleAudiences.map((audience) => <section key={audience.key} className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
