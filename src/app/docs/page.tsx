...
const groups = [
  {
    ...
    description: "Learn how to monetize channels, Mini Apps, and bots with AdsGalaxy.",
    href: "/docs/publisher",
    items: [
  ...
  {
    title: "Advertiser Guides",
    description: "Create campaigns for channels, Mini Apps, and bot broadcasts.",
  ...
  },
  {
    ...
    description: "Integrate AdsGalaxy into Mini Apps, bots, websites, mobile apps, and future platforms.",
    ...
    <div className="space-y-8">
      <section className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
          Practical guides for publishers and advertisers using AdsGalaxy across Telegram channels, Mini Apps, and bots.
        </p>
      </section>
          ...
          <section key={group.title} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 space-y-2">
              {group.items.map((item) => (
                <Link key={item.href} href={item.href} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-3 text-sm font-bold text-slate-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-600">
                  <item.icon size={18} />
                  {item.label}
                </Link>
              ))}
