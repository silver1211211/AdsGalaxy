...
function sdkSource(defaultMiniappId: string, requestOrigin: string) {
  ...
  function showInternalAd(decision,options){
    ...
    return new Promise(function(resolve){
      ...
      var displayTitle=String(ad.title||"").trim().slice(0,20),displayDesc=String(ad.description||"").trim().slice(0,80),displayCta=String(ad.cta_text||"Learn More").trim().slice(0,24)||"Learn More",logoUrl=String(ad.advertiser_logo_url||ad.logo_url||"").trim();
      var overlay=document.createElement("div");overlay.className="agx-rewarded-overlay";
      var style=document.createElement("style");style.textContent=".agx-rewarded-overlay{position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;padding:calc(env(safe-area-inset-top) + 18px) max(18px,env(safe-area-inset-right)) calc(env(safe-area-inset-bottom) + 18px) max(18px,env(safe-area-inset-left));font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#f8fafc;background:radial-gradient(circle at 22% 8%,rgba(99,102,241,.34),transparent 24%),radial-gradient(circle at 82% 16%,rgba(34,211,238,.18),transparent 24%),linear-gradient(180deg,#020617 0%,#07101e 48%,#020617 100%);animation:agxRewardedFade .18s ease-out;overflow:hidden}.agx-rewarded-overlay:before{content:'';position:absolute;inset:0;background-image:radial-gradient(circle,rgba(255,255,255,.2) 0 1px,transparent 1.5px),radial-gradient(circle,rgba(125,211,252,.16) 0 1px,transparent 1.4px);background-size:92px 92px,137px 137px;background-position:12px 18px,48px 60px;opacity:.34;pointer-events:none}.agx-rewarded-overlay:after{content:'';position:absolute;inset:0;background:linear-gradient(115deg,transparent 0 24%,rgba(255,255,255,.045) 36%,transparent 50% 100%);pointer-events:none}.agx-rewarded-card{position:relative;z-index:1;width:min(520px,92vw);max-height:calc(100dvh - 36px);overflow:auto;border-radius:24px;padding:18px 18px 14px;background:linear-gradient(180deg,rgba(17,24,39,.98),rgba(5,11,23,.98));border:1px solid rgba(203,213,225,.16);box-shadow:0 30px 100px rgba(0,0,0,.66),0 0 0 1px rgba(99,102,241,.08),0 0 48px rgba(79,70,229,.2),inset 0 1px 0 rgba(255,255,255,.08);text-align:center;animation:agxRewardedPop .24s cubic-bezier(.2,.8,.2,1);overscroll-behavior:contain}.agx-rewarded-top{position:relative;display:flex;align-items:center;justify-content:center;min-height:36px;margin-bottom:12px}.agx-rewarded-heading{font-size:18px;line-height:1.1;font-weight:900;color:#f8fafc}.agx-rewarded-close{position:absolute;right:0;top:50%;width:36px;height:36px;transform:translateY(-50%);border-radius:999px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.08);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:28px;line-height:1;font-weight:300;cursor:pointer}.agx-rewarded-close[hidden]{display:none}.agx-rewarded-media{display:flex;align-items:center;justify-content:center;width:100%;min-height:150px;max-height:min(45dvh,300px);border-radius:18px;background:linear-gradient(135deg,rgba(15,23,42,.92),rgba(30,41,59,.74));border:1px solid rgba(203,213,225,.12);box-shadow:inset 0 1px 0 rgba(255,255,255,.05),0 18px 42px rgba(0,0,0,.22);overflow:hidden;cursor:pointer}.agx-rewarded-hero{display:block;width:100%;height:100%;max-height:min(45dvh,300px);object-fit:contain;background:#eaf5ff}.agx-rewarded-body{padding:16px 2px 0;text-align:center}.agx-rewarded-logo{width:34px;height:34px;border-radius:10px;object-fit:cover;border:1px solid rgba(255,255,255,.16);background:#0f172a;margin-bottom:10px}.agx-rewarded-title{margin:0 auto 8px;max-width:100%;font-size:21px;line-height:1.2;font-weight:950;color:#fff;overflow-wrap:anywhere}.agx-rewarded-desc{margin:0 auto 16px;max-width:380px;color:#b9c3d5;font-size:14px;line-height:1.45;font-weight:650;overflow-wrap:anywhere}.agx-rewarded-cta{width:100%;min-height:52px;border:0;border-radius:15px;background:linear-gradient(135deg,#5b5cff 0%,#2563eb 52%,#14b8a6 100%);color:#fff;font-size:16px;font-weight:950;padding:14px 18px;cursor:pointer;box-shadow:0 16px 34px rgba(37,99,235,.34),inset 0 1px 0 rgba(255,255,255,.18)}.agx-rewarded-sponsored{display:flex;align-items:center;justify-content:center;gap:9px;margin:18px 0 14px;padding-top:14px;border-top:1px solid rgba(148,163,184,.12);text-decoration:none;font-size:14px;font-weight:800;color:#818ca1}.agx-rewarded-mark{position:relative;display:inline-flex;width:28px;height:28px;border-radius:999px;align-items:center;justify-content:center;background:conic-gradient(from 140deg,#22d3ee,#5b5cff,#8b5cf6,#22d3ee);box-shadow:0 0 22px rgba(91,92,255,.26)}.agx-rewarded-mark:before{content:'';position:absolute;inset:6px;border:2px solid rgba(255,255,255,.88);border-right-color:transparent;border-radius:999px;transform:rotate(-22deg)}.agx-rewarded-mark:after{content:'';position:absolute;width:5px;height:5px;border-radius:999px;background:#fff;box-shadow:0 0 10px rgba(255,255,255,.7);transform:translate(8px,-7px)}.agx-rewarded-sponsored strong{color:#7477ff}.agx-rewarded-countdown{display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid rgba(148,163,184,.18);border-radius:16px;background:rgba(15,23,42,.48);padding:12px 14px 12px 16px}.agx-rewarded-countdown[hidden]{display:none}.agx-rewarded-countdown-label{display:flex;align-items:center;gap:10px;color:#9aa5b8;font-size:15px;font-weight:800}.agx-rewarded-ring-wrap{position:relative;width:50px;height:50px;flex:0 0 auto}.agx-rewarded-ring{width:50px;height:50px;transform:rotate(-90deg)}.agx-rewarded-ring circle{fill:none;stroke-width:5}.agx-rewarded-ring-track{stroke:rgba(148,163,184,.18)}.agx-rewarded-ring-progress{stroke:#5b5cff;stroke-linecap:round;transition:stroke-dashoffset .28s linear}.agx-rewarded-ring-text{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#fff;font-size:17px;font-weight:950;font-variant-numeric:tabular-nums}.agx-rewarded-watermark{margin-top:12px;color:rgba(203,213,225,.48);font-size:12px;font-weight:800}@keyframes agxRewardedFade{from{opacity:0}to{opacity:1}}@keyframes agxRewardedPop{from{opacity:0;transform:translate3d(0,10px,0) scale(.975)}to{opacity:1;transform:translate3d(0,0,0) scale(1)}}@media (orientation:landscape) and (max-height:560px){.agx-rewarded-card{width:min(640px,92vw);max-height:calc(100dvh - 24px);padding:14px}.agx-rewarded-media,.agx-rewarded-hero{max-height:36dvh}.agx-rewarded-body{padding-top:10px}.agx-rewarded-sponsored{margin:12px 0 10px}}";
      var panel=document.createElement("div");panel.className="agx-rewarded-card";panel.setAttribute("role","dialog");panel.setAttribute("aria-modal","true");panel.setAttribute("aria-label","Sponsored ad");
      ...
      if(ad.image_url){var media=document.createElement("div");media.className="agx-rewarded-media";media.onclick=openAd;var img=document.createElement("img");img.src=ad.image_url;img.alt=displayTitle;img.loading="eager";img.decoding="async";img.className="agx-rewarded-hero";media.appendChild(img);panel.appendChild(media);}
      var body=document.createElement("div");body.className="agx-rewarded-body";if(logoUrl){var logo=document.createElement("img");logo.src=logoUrl;logo.alt="";logo.className="agx-rewarded-logo";body.appendChild(logo);}var title=document.createElement("div");title.textContent=displayTitle;title.className="agx-rewarded-title";if(ad.title_color)title.style.color=ad.title_color;var desc=document.createElement("div");desc.textContent=displayDesc;desc.className="agx-rewarded-desc";if(ad.body_color)desc.style.color=ad.body_color;var cta=document.createElement("button");cta.type="button";cta.textContent=displayCta;cta.className="agx-rewarded-cta";cta.onclick=openAd;body.appendChild(title);body.appendChild(desc);body.appendChild(cta);panel.appendChild(body);
      var sponsor=document.createElement("a");sponsor.href=botUrl;sponsor.target="_blank";sponsor.rel="noopener noreferrer";sponsor.className="agx-rewarded-sponsored";sponsor.innerHTML="<span class='agx-rewarded-mark' aria-hidden='true'></span><span>Sponsored by</span><strong>AdsGalaxy</strong>";
      ...
      var watermark=document.createElement("div");watermark.className="agx-rewarded-watermark";watermark.textContent="@Ads_Galaxy_bot";panel.appendChild(sponsor);panel.appendChild(countdownBox);panel.appendChild(watermark);overlay.appendChild(style);overlay.appendChild(panel);document.body.style.overflow="hidden";document.body.appendChild(overlay);
