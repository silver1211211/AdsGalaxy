...
function baseSelect(type: MarketplaceInventoryType) {
  ...
  return `
    SELECT
      ...
      COALESCE((SELECT COUNT(*) FROM broadcast_deliveries bd WHERE bd.bot_id = b.id AND bd.status = 'sent' AND bd.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)), 0) as calculated_monthly_impressions,
      0 as calculated_completion_rate,
