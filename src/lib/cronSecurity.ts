...
import { acquireRedisLock, releaseRedisLock, type RedisLock } from "@/lib/redisLock";

type CronLock = { lockName: string; ownerToken: string; redisLock?: RedisLock | null };
...
export async function acquireCronLock(lockName: string, ttlSeconds = 900) {
  ...
  const redisAttempt = await acquireRedisLock(redisKeys.cronLock(lockName), ttlSeconds * 1_000);
  if (redisAttempt.status === "busy") return null;
  const redisLock = redisAttempt.status === "acquired" ? redisAttempt.lock : null;

  try {
    // The durable database lock remains authoritative. Redis is a fast pre-gate
    // that keeps duplicate cron requests from repeatedly reaching MariaDB.
    await pool.query(
    ...
    return { lockName, ownerToken, redisLock } satisfies CronLock;
  } catch (error) {
    await releaseRedisLock(redisLock);
    throw error;
  }
...
export async function releaseCronLock(lock: CronLock | null) {
  if (!lock) return;
  try {
    await pool.query(
    ...
    await releaseRedisLock(lock.redisLock ?? null);
  }
}
