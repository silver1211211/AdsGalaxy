  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-white flex items-center justify-center">
      <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-center shadow-2xl shadow-black/30">
        ...
        <h1 className="text-2xl font-black tracking-tight">{t("auth.banned.suspended")}</h1>
        <p className="mt-3 text-sm font-medium leading-6 text-slate-300">
          {t("auth.banned.suspendedDescription")}
        </p>
        ...
        >
          {t("auth.banned.supportLabel", { username: `@${SUPPORT_USERNAME}` })}
        </a>
        <a
          href={SUPPORT_URL}
          target="_blank"
        ...
        >
          {t("auth.banned.messageSupport", { username: `@${SUPPORT_USERNAME}` })}
        </a>
      </div>
    </main>
  );
